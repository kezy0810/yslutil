...
public class Validation {  
    ...
    /** 
     * 邮编正则表达式  [0-9]\d{5}(?!\d) 国内6位邮编 (有问题应该为\d{6}(?!\d)   )
     */
    ...
    /*1、正则表达式验证数字、字母、特殊字符的组合，这个要求3者都有(不区分大小写)
    var reg=/(?=.*[a-z])(?=.*\d)(?=.*[#@!~%^&*])[a-z\d#@!~%^&*]{8,16}/i
          特殊 字符 为~!@#$%^&*其中之一

    2、正则表达式验证（数字+字母）或者（数字+特殊字符）或者（字母+特殊字符），不能是纯数字、纯字母、纯特殊字符，即只要符合这3个组合其中之一都为true(不区分大小写)
    var reg=/((?=.*[a-z])(?=.*\d)|(?=[a-z])(?=.*[#@!~%^&*])|(?=.*\d)(?=.*[#@!~%^&*]))[a-z\d#@!~%^&*]{8,16}/i*/
    /**
     * 匹配6-20为字母、数字、特殊字符组成的密码（至少其中两种组合方式）
     */
    public static final String ISPWD = "((?=.*[a-zA-Z])(?=.*\\d)|(?=[a-zA-Z])(?=.*[#@!~%^&*])|(?=.*\\d)(?=.*[#@!~%^&*]))[a-zA-Z\\d#@!~%^&*]{6,20}";

...
public class YCValidatorUtil {
	...
	public static <E> List<ErrorMsg> valide(E e) throws Exception {
		...
		while (it.hasNext()) {
			...
			errorMsg.setInvalidValue(tVilidateRs.getInvalidValue() != null ? tVilidateRs
					.getInvalidValue().toString() : "");
			ConstraintDescriptor<?> t = tVilidateRs.getConstraintDescriptor();
			Map<String, Object> s = t.getAttributes();
			errorMsg.setRegexp(s.get("regexp") != null ? s.get("regexp")
					.toString() : "");
			boolean flags = true;
			if (errorMsg
					.getRegexp()
					.equals("^[0-9]\\d*$|^([0-9]{0,1}(\\.[0-9][0-9]?))$|^([1-9][0-9]*(\\.[0-9][0-9]?))$")) {// 大于或等于零、最多两位小数的阿拉伯数字正则表达式
				...
				if (boo) {
					...
					String mothanme = "set"
							+ errorMsg.getPropertyPath().replaceFirst(
									...
									errorMsg.getPropertyPath().substring(0, 1)
											.toUpperCase());
					e.getClass().getMethod(mothanme, String.class)
							.invoke(e, df.format(Double.parseDouble(value)));
					flags = false;
				}
			}
			...
			if(errorMsg.getRegexp().equals("^-?[0-9]\\d*$|^-?([0-9]{0,1}(\\.[0-9][0-9]?))$|^-?([1-9][0-9]*(\\.[0-9][0-9]?))$")){//大于或等于零、最多四位小数的阿拉伯数字正则表达式
				String value = errorMsg.getInvalidValue();
			    ...
			    if(boo){
			    	DecimalFormat df = new DecimalFormat("#.00"); 
			        System.out.println(df.format(Double.parseDouble(value))); 
			        String mothanme = "set"+errorMsg.getPropertyPath().replaceFirst(errorMsg.getPropertyPath().substring(0, 1),errorMsg.getPropertyPath().substring(0, 1).toUpperCase());
	 ...
	 * @throws Exception
	 */
	public static <E, P> List<ErrorMsg> valideMaxValue(E bean, P p)
			throws Exception {
		List<ErrorMsg> errorMsgList = new ArrayList<ErrorMsg>();
		...
		for (Field field : fields) {
			try {
				...
				Method fieldGetMet = pcls.getMethod(fieldGetName,
						new Class[] {});
				...
				if (fieldVal != null) {
					...
					if (type.endsWith("Integer")) {
						...
						if (temp.length() > 10) {
							...
							if (String.valueOf(Integer.MAX_VALUE).compareTo(
									temp) < 0) {
								errorMsgList.add(init(pfields,temp,message+Integer.MAX_VALUE+"!"));
							}
						}
					...
					if (type.endsWith("Long")) {
						...
						if (temp.length() > 19) {
							...
							if (String.valueOf(Long.MAX_VALUE).compareTo(
									temp) < 0) {
								errorMsgList.add(init(pfields,temp,message+Long.MAX_VALUE+"!"));
							}
						}
	...
	public static ErrorMsg init(Field field,String temp,String message) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		String values="";
		ErrorMsg errorMsg = new ErrorMsg();
		if (field.isAnnotationPresent(Pattern.class)) {
			Annotation ann = field
					.getAnnotation(Pattern.class);
			Method m = ann.getClass()
					.getDeclaredMethod("message", null);
